...
module Util = struct
  ...
  let get_JWK_x5t fingerprint =
    U_Base64.url_encode ~len:20 fingerprint
  ...
  let get_ES256_x_y =
    get_ESXXX_x_y ~split_at:32 (* 64 octets split in 2 *)
      ~pub_to_cstruct:Mirage_crypto_ec.P256.Dsa.pub_to_octets

  let make_ES256_of_x_y =
    make_ESXXX_of_x_y ~pub_of_cstruct:Mirage_crypto_ec.P256.Dsa.pub_of_octets

  let get_ES384_x_y =
    get_ESXXX_x_y ~split_at:48 (* 96 octets split in 2 *)
      ~pub_to_cstruct:Mirage_crypto_ec.P384.Dsa.pub_to_octets

  let make_ES384_of_x_y =
    make_ESXXX_of_x_y ~pub_of_cstruct:Mirage_crypto_ec.P384.Dsa.pub_of_octets

  let get_ES512_x_y =
    get_ESXXX_x_y ~split_at:66 (* 132 octets split in 2 *)
      ~pub_to_cstruct:Mirage_crypto_ec.P521.Dsa.pub_to_octets

  let make_ES512_of_x_y =
    make_ESXXX_of_x_y ~pub_of_cstruct:Mirage_crypto_ec.P521.Dsa.pub_of_octets
end
...
let to_pub_pem (type a) (jwk : a t) =
  match jwk with
  | Rsa_pub rsa ->
    Ok (X509.Public_key.encode_pem (`RSA rsa.key))
  | Rsa_priv rsa ->
    rsa.key |> Mirage_crypto_pk.Rsa.pub_of_priv
    |> (fun key -> X509.Public_key.encode_pem (`RSA key))
    |> Result.ok
  | Es256_pub ec ->
    Ok (X509.Public_key.encode_pem (`P256 ec.key))
  | Es256_priv ec ->
    ec.key |> Mirage_crypto_ec.P256.Dsa.pub_of_priv
    |> (fun key ->
         X509.Public_key.encode_pem (`P256 key))
    |> Result.ok
  | Es384_pub ec ->
    Ok (X509.Public_key.encode_pem (`P384 ec.key))
  | Es384_priv ec ->
    ec.key |> Mirage_crypto_ec.P384.Dsa.pub_of_priv
    |> (fun key ->
         X509.Public_key.encode_pem (`P384 key))
    |> Result.ok
  | Es512_pub ec ->
    Ok (X509.Public_key.encode_pem (`P521 ec.key))
  | Es512_priv ec ->
    ec.key |> Mirage_crypto_ec.P521.Dsa.pub_of_priv
    |> (fun key ->
         X509.Public_key.encode_pem (`P521 key))
    |> Result.ok
  | _ -> Error `Unsupported_kty
...
let to_priv_pem (jwk : priv t) =
  match jwk with
  | Rsa_priv rsa ->
      Ok (X509.Private_key.encode_pem (`RSA rsa.key))
  | Es256_priv ec ->
      Ok (X509.Private_key.encode_pem (`P256 ec.key))
  | Es384_priv ec ->
      Ok (X509.Private_key.encode_pem (`P384 ec.key))
  | Es512_priv ec ->
      Ok (X509.Private_key.encode_pem (`P521 ec.key))
  | _ -> Error `Unsupported_kty
...
let priv_esXXX_to_priv_json ~get_ESXXX_x_y ~pub_of_priv ~priv_to_cstruct ~crv
  ...
  let d =
    priv_to_cstruct priv.key |> U_Base64.url_encode_string
  in
  let values =
    [
...
let priv_es256_to_priv_json =
  priv_esXXX_to_priv_json ~get_ESXXX_x_y:Util.get_ES256_x_y
    ...
    ~priv_to_cstruct:Mirage_crypto_ec.P256.Dsa.priv_to_octets ~crv:"P-256"
...
let priv_es384_to_priv_json =
  priv_esXXX_to_priv_json ~get_ESXXX_x_y:Util.get_ES384_x_y
    ...
    ~priv_to_cstruct:Mirage_crypto_ec.P384.Dsa.priv_to_octets ~crv:"P-384"
...
let priv_es512_to_priv_json =
  priv_esXXX_to_priv_json ~get_ESXXX_x_y:Util.get_ES512_x_y
    ...
    ~priv_to_cstruct:Mirage_crypto_ec.P521.Dsa.priv_to_octets ~crv:"P-521"
...
let pub_okp_of_json json =
  ...
  try
    ...
    Result.bind x (fun cstruct ->
         Mirage_crypto_ec.Ed25519.pub_of_octets cstruct
         |> Result.map_error (fun _ -> `Msg "Could not create key"))
...
let priv_okp_of_json json =
  ...
  try
    ...
    Result.bind d (fun cstruct ->
           Mirage_crypto_ec.Ed25519.priv_of_octets cstruct
           |> Result.map_error (fun _ -> `Msg "Could not create key"))
