...
module Jwa : sig
  type alg =
    ...
    | `ECDH_ES_A256KW
      (** ECDH-ES using Concat KDF and CEK wrapped with "A256KW" - Recommended
          ({{:https://www.rfc-editor.org/info/rfc7518/#section-4.6} RFC 7518
            §4.6},
          {{:https://www.rfc-editor.org/info/rfc7518/#section-4.6} §4.6}) *)
    | `None
  ...
  (** {{:https://tools.ietf.org/html/rfc7518#section-3.1} Link to RFC 7518 §3.1}
      ...
      - Key management algorithms supported: [RSA1_5], [RSA_OAEP], [Dir],
        [A128KW], [A256KW], [ECDH_ES], [ECDH_ES_A128KW], [ECDH_ES_A256KW]. *)
  ...
  type use =
    [ `Sig
      (** Key used for digital signature or MAC
          ({{:https://www.rfc-editor.org/info/rfc7517/#section-4.2} RFC 7517
            §4.2}) *)
    | `Enc
...
module Jwk : sig
  ...
  *)
  ...
  val make_pub_rsa :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_pk.Rsa.pub -> public t
  (** [make_pub_rsa ?use ?alg pub] takes a public key generated by mirage-crypto
      and returns a [public t]. *)

  val make_pub_es256 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.P256.Dsa.pub -> public t
  (** [make_pub_es256 ?use ?alg pub] takes a P-256 public key and returns a
      [public t]. *)

  val make_pub_es384 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.P384.Dsa.pub -> public t
  (** [make_pub_es384 ?use ?alg pub] takes a P-384 public key and returns a
      [public t]. *)

  val make_pub_es512 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.P521.Dsa.pub -> public t
  (** [make_pub_es512 ?use ?alg pub] takes a P-521 public key and returns a
      [public t]. *)

  val make_pub_ed25519 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.Ed25519.pub -> public t
  (** [make_pub_ed25519 ?use ?alg pub] takes an Ed25519 public key and returns a
      [public t]. *)
  ...
  (** [of_pub_pem ?use ?alg pem] takes a PEM as a string and returns a
      [public t] or a message of what went wrong. *)
  ...
  *)
  ...
  val make_priv_rsa :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_pk.Rsa.priv -> priv t
  (** [make_priv_rsa ?use ?alg priv] takes a private key generated by
      mirage-crypto and returns a [priv t]. *)

  val make_priv_es256 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.P256.Dsa.priv -> priv t
  (** [make_priv_es256 ?use ?alg priv] takes a P-256 private key and returns a
      [priv t]. *)

  val make_priv_es384 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.P384.Dsa.priv -> priv t
  (** [make_priv_es384 ?use ?alg priv] takes a P-384 private key and returns a
      [priv t]. *)

  val make_priv_es512 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.P521.Dsa.priv -> priv t
  (** [make_priv_es512 ?use ?alg priv] takes a P-521 private key and returns a
      [priv t]. *)

  val make_priv_ed25519 :
    ?use:Jwa.use -> ?alg:Jwa.alg -> Mirage_crypto_ec.Ed25519.priv -> priv t
  (** [make_priv_ed25519 ?use ?alg priv] takes an Ed25519 private key and
      returns a [priv t]. *)
  ...
  (** [of_priv_pem ?use ?alg pem] takes a PEM as a string and returns a [priv t]
      or a message of what went wrong. *)

  val make_oct : ?use:Jwa.use -> ?alg:Jwa.alg -> string -> priv t
