    ...
    [
      ( "JWKs",
        [
          ...
          Alcotest.test_case "Can get correct JWK from Microsoft" `Quick (fun () ->
            let jwks =
              Jose.Jwks.of_string microsoft_jwks_string
            in
            check_string "Should find key" microsoft_jwk_kid
              (Jose.Jwks.find_key jwks microsoft_jwk_kid |> CCOpt.get_exn |> Jose.Jwk.get_kid));
        ] );
    ]
